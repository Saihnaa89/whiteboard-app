...
export class ChatEvents {
  ...
  private async setupChatEvents() {
    this.io.on('connection', (socket: Socket) => {
      // Handle new messages
      socket.on(SocketEvents.MESSAGE, async (data: MessageData) => {
        try {
      ...
      socket.on(SocketEvents.TYPING, (data: { roomId: string }) => {
        try {
          const { roomId } = data;
      ...
      socket.on(SocketEvents.STOPPED_TYPING, (data: { roomId: string }) => {
        try {
          const { roomId } = data;
